...
{
public:
	int a[2]; // could be private, but methods would be needed to access this member.
...
int main()
{
	...
	if(obj2 > obj1)	// Which operator is being called? //operator> called
	...
	obj1 + obj2;	// normal expression. Which operator is being called? //operator+ called
	obj1.operator+(obj2);	// equivalent, try it out
	obj1.Printvals();
	obj2.Printvals();

	obj2 + obj1;	// normal expression. Which operator is being called? //operator+ called
	obj2.operator+(obj1);	// equivalent, try it out
	obj1.Printvals();
	obj2.Printvals();

	cout << "------------------------------------------------------------------" << endl;
	obj1*2;		// Would 2*obj1 work? Give it a try. //no, compile error
	obj2*f;		// Would f*obj2 work? Give it a try. //no, compile error
	obj1.Printvals();
	obj2.Printvals();
